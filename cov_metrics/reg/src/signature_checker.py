...
from src.riscv_definitions import *

class sigChecker():
    def __init__(self, num_inputs, isa_sigdir, rtl_sigdir):
        self.num_inputs = num_inputs
        self.isa_sigdir = isa_sigdir
        self.rtl_sigdir = rtl_sigdir
    ...
    # def check_all(self, symbolss, isa_sigdir, rtl_sigdir):
        # matches = []
        # for i in range(self.num_inputs):
            # matches.append(check(symbolss[i], isa_sigdir + str(i), rtl_sigdir + str(i)))

        # return matches

    def check(self, symbols, isa_sigfile, rtl_sigfile):

        (xreg_idxes, freg_idxes, csr_idxes, data_symbols, data_idx_start) = \
            self.read_symbols(symbols)

        (isa_xreg_vals, isa_freg_vals, isa_csr_vals, isa_data_vals) = \
            self.read_sig(isa_sigfile, xreg_idxes, freg_idxes,
                          csr_idxes, data_symbols, data_idx_start)

        (rtl_xreg_vals, rtl_freg_vals, rtl_csr_vals, rtl_data_vals) = \
            self.read_sig(rtl_sigfile, xreg_idxes, freg_idxes,
                          csr_idxes, data_symbols, data_idx_start)
