    ...
    def read_ga(self, ga_name):
        ...
        while True:
            ...
            if 'data:' in line:
                ...
                self.inst_generator.prefix_num = num_prefix
                self.inst_generator.main_num = num_word
                self.inst_generator.suffix_num = num_suffix

                if num_prefix < self.num_prefix:
                    part = PREFIX
                    num_prefix += 1
                    tmp_tuples = self.read_label(line, prefix_tuples)
                    num_tmp = num_prefix
                ...
                elif num_suffix < self.num_suffix:
                    part = SUFFIX
                    num_suffix += 1
                    tmp_tuples = self.read_label(line, suffix_tuples)
                    num_tmp = num_suffix
                ...
                for i in range(len(insts)):
                    tmp_tuples[num_tmp - 1][1].append(insts[i][8:50])
                    if part == MAIN:
                        # MAIN
